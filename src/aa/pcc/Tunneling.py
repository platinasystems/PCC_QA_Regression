...
class Tunneling(AaBase):
    ...
    ###############################################################################################################
    @keyword(name="Add mutliple nodes and check online")
    ###############################################################################################################   
    ...
    def add_multiple_nodes_and_check_online(self, *args, **kwargs):
        ...
        try:
            ...
            for hostip in ast.literal_eval(self.host_ips):
                add_node_status = Nodes().add_node(Host=hostip, managed= self.managed, standby = self.standby)
                
            for name in ast.literal_eval(self.Names):
                verify_node_online_status = Nodes().wait_until_node_ready(Name=name)
    ...
    ###############################################################################################################
    @keyword(name="Delete mutliple nodes and wait until deletion")
    ###############################################################################################################   
    ...
    def delete_multiple_nodes_and_wait_until_deletion(self, *args, **kwargs):
        ...
        try:
            if self.Names:
                deletion_status = []
                for name in ast.literal_eval(self.Names):
                    node_id= Nodes().get_node_id(Name=name)
                    banner(node_id)
                    delete_node_status = Nodes().delete_node(Id=str(node_id))
                    logger.console(delete_node_status)
                        
                for name in ast.literal_eval(self.Names):
                    deletion_response = Nodes().wait_until_node_deleted(Name=name)
                ...
                if get_response_data(response) == []:
                    ...
                    for id_ in list_id:
                        response = Nodes().delete_node(Id=str(id_))
